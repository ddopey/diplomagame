...
class Actor {
  constructor(position = new Vector(0, 0), size = new Vector(1, 1), speed = new Vector(0, 0)) {
    if ((!(position instanceof Vector)) || (!(size instanceof Vector)) || (!(speed instanceof Vector))) {
      throw new Error('В качестве аргумента передан не вектор типа Vector');
    }
    this.pos = position;
    this.size = size;
    this.speed = speed;
  ...
  isIntersect(actor) {
    if (!(actor instanceof Actor)) {
    	throw new Error('Ошибка при передаче аргумента');
    }
    if (actor === this) {
    	return false;
    }
  ...
  createActors(plan) {
    return plan.reduce((memo, el, i) => {
    	el.split('').forEach((item, k) => {
    		const Constr = this.actorFromSymbol(item);
    		if (typeof Constr === 'function') {
    			const obj = new Constr(new Vector(k, i));
